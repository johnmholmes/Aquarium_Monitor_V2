...
void setup() {
  ...
  mqttClient.subscribe("backupMaxSetting", 1);
  mqttClient.subscribe("backupMinSetting", 1);
...
void onMqttMessage(char* topic, char* payload, AsyncMqttClientMessageProperties properties, size_t len, size_t index, size_t total) {
  if (strcmp(topic, "backupMaxSetting") == 0) {
    float newMaxTemp = atof(payload);  // Convert the payload to a floating-point number
  ...
  if (strcmp(topic, "backupMinSetting") == 0) {
    float newMinTemp = atof(payload);  // Convert the payload to a floating-point number
